...
2. Add the `fix-me` label to the issue or leave a comment on the issue starting with `@Ab0amin`.
...
1. Create an issue in the repository.
2. Add the `fix-me` label to the issue, or leave a comment starting with `@Ab0amin`.
...
5. Add the `fix-me` label to the pull request, or address a specific comment by starting with `@Ab0amin`.

### Label versus Macro

- Label (`fix-me`): Requests OpenHands to address the **entire** issue or pull request.
- Macro (`@Ab0amin`): Requests OpenHands to consider only the issue/pull request description and **the specific comment**.
